function [net,Z,P] = snn_sample_ctrfm( net, data, ct )
% Draws a sequence Z - continuous time spiking hmm multi sems with refractory mechanim.
%
...
%   num_groups      1            number of wta groups
...
%   At    szeros  [num_groups,0]    log activity of network at spike times
%   hX    zeros   [num_inputs,2]    feedforward PSPs
%   hZ    zeros   [num_neurons,2]   recurrent PSPs
%   R     zeros   [num_groups,1]    current importance weight
...
%   
    ...
    num_spikes = poissrnd( double( net.num_groups*net.lambda*time_range ) );
    ...
    net.At = zeros(net.num_groups+1,num_spikes, 'single');
    ...
    try
        ...
        for j = 1:num_spikes
            ...
            U = net.W*d_hX + net.V*d_hZ + net.V0 + u_rf;
            
            [P_t,A] = wta_softmax( reshape( U, [], net.num_groups ) );

            [Z_i,k] = wta_draw_k( P_t(:)/net.num_groups );
